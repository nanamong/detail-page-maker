...
export async function POST(request: NextRequest) {
  try {
    ...
    const productInfoText = String(form.get("productInfoText") || "").slice(0, 30000);
    ...
    const backgroundColor = String(form.get("backgroundColor") || "auto");
...
async function analyzeSource({
  ...
  const prompt = [
    ...
    "업로드된 기존 상세페이지 이미지(레퍼런스)에서는 오직 [디자인 톤앤매너와 레이아웃 구조]만 벤치마킹하라. 제품컷을 제외한 실제 문구, 텍스트 내용은 절대 레퍼런스에서 가져오지 마라.",
    ...
    `optimal_sections_count: 제공된 상품 정보와 레퍼런스를 분석하여, 상세페이지를 완벽하게 구성하기 위해 필요한 최적의 섹션 개수를 숫자로 반환하라. 사용자가 선택한 범위에 따라 반드시 ${payload.options.count === 4 ? "3에서 5 사이" : payload.options.count === 7 ? "6에서 8 사이" : payload.options.count === 10 ? "9에서 11 사이" : "1"}의 정수여야 한다.`,
...
async function generateOpenAIImage({ apiKey, prompt, references }: { apiKey: string; prompt: string; references: ReferenceImage[] }) {
  ...
  form.  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${GOOGLE_NANO_BANANA_2_MODEL}:generateContent`, {
    method: "POST",
    headers: {
      "x-goog-api-key": apiKey,
      "Content-Type": "application/json"
  ...
  const data = await readJsonResponse(response);
  if (!response.ok) throw new Error(withRequestId(data?.error?.message || "Google 생성 실패", response));
...
function buildSections(
  ...
  payload: { request: string; rolloutRequest: string; knowledgeText: string; productInfoText: string; options: { channel: string; ratio: string; count: number; backgroundColor: string } },
  analysis: unknown,
  modelInfo: { provider: string; label: string; id: string }
) {
  const bgColorInstruction = payload.options.backgroundColor === "black" 
    ? "이 이미지는 무조건 다크모드(블랙/어두운 톤) 배경으로 생성해야 한다."
    : payload.options.backgroundColor === "white" 
      ? "이 이미지는 무조건 화이트(밝은 톤) 배경으로 생성해야 한다."
      : "배경 컬러는 레퍼런스를 분석하여 자동으로 결정하거나 이미지를 별도로 제공할 경우 사용자가 요청하는 컬러의 톤앤매너를 우선으로 제작한다.";
  ...
  return sectionTemplates(count, startSection).map(template => {
    const promptText = [
      ...
      startSection > 1 
        ? "🚨 [절대 규칙] 첨부된 이미지들은 이전에 생성된 앞선 섹션들입니다. 이번에 생성할 새 이미지는 무조건 첨부된 이미지들과 완벽하게 동일한 톤앤매너(배경색, 폰트 느낌, 포인트 컬러, 버튼 스타일)를 유지하여 '하나의 긴 상세페이지'로 이어지게 하세요. 절대로 쌩뚱맞은 다른 컬러나 새로운 디자인 컨셉을 도입하지 마세요."
